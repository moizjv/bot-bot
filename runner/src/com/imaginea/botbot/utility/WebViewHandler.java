package com.imaginea.botbot.utility;
...
public class WebViewHandler {
	...
	public static void clickOnLinkInWebView(WebView browser, String link,Solo solo) {
		...
		browser.loadUrl("javascript:links=document.getElementsByTagName('a');var linkPresent=false;"
							+ "for(var i=0;i<links.length;i++){"
								+ "if(links[i].innerHTML=='"+ link +"'){ linkPresent=true;"
									+ "var event = document.createEvent('MouseEvents');"
									+ "event.initMouseEvent('click', true, window, 1, 0, 0, 0, 0, false, false, false, 1, document.body.parentNode);"
								+ "links[i].dispatchEvent(event);" + "}" + "} if(linkPresent==false){console.log('ConsoleMessage'+'LinkNotPresent');}");
	...
	public static boolean isTextPresentInWebView(WebView browser,String text,Solo solo)
	{
